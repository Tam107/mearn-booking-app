...
const AdminViewBus = () => {
    ...
    // Get min & max prices
    const prices = stateBus?.busesAdmin?.map((b) => b.price) || [];
    ...
    const filteredBuses = useMemo(() => {
        ...
        return buses;
    }, [searchText, cityFilter, priceRange, stateBus?.busesAdmin]);

    const columns = [
        // { title: "ID", dataIndex: "_id", key: "_id", width: 220, ellipsis: true },
        { title: "Bus Name", dataIndex: "poName", key: "poName", width: 250, ellipsis: true },
        { title: "City From", dataIndex: "cityFrom", key: "cityFrom", width: 150, ellipsis: true },
        { title: "City To", dataIndex: "cityTo", key: "cityTo", width: 150, ellipsis: true },
        {
            ...
            {isMobile ? (
                <div className="space-y-4">
                    {filteredBuses.map((bus) => (
                        <div key={bus._id} className="border rounded-lg p-4 mb-3 shadow-sm bg-white">
                            <p><strong>Bus Name:</strong> {bus.poName}</p>
                            <p><strong>From:</strong> {bus.cityFrom}</p>
                            <p><strong>To:</strong> {bus.cityTo}</p>
